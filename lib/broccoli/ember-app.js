...
class Compile extends BroccoliPlugin {
  ...
  build() {
    ...
    let addons = this.options.addons;
    let babelConfig = this.options.babelConfig;
    let entryPoint = this.options.entryPoint;
    // console.log(JSON.stringify(babelConfig))

    let babelAddon = addons.find(addon => addon.name === 'ember-cli-babel');

    // create the babel options to use elsewhere based on the config above
    let babelOptions = babelAddon.buildBabelOptions(babelConfig);
    delete babelOptions.annotation;
    babelOptions.plugins = babelOptions.plugins.slice(1);
    ...
    // let b = path.join(path.dirname(require.resolve('rollup-src')), '../Bundle');
    ...
    // // let require2 = require("@std/esm")(module)

    // console.log(b);

    const Bundle = require('rollup-src');
    const resolve = require('rollup-plugin-node-resolve');
    const replace = require('rollup-plugin-replace');

    let bundle = new Bundle({
      input: path.join(inputPath, entryPoint),
      plugins: [
        replace({
          delimiters: ['', ''],
          // './config/environment': './config/environments/development.json',
          './config/environment': 'config-environments-development-json',
        }),
        resolve({
    ...
    for (let file of files) {
      ...
      let precompiledSource = fs.readFileSync(filePath, 'utf8');
      babelOptions.filename = file;
      let ast = require('babylon').parse(precompiledSource, { sourceType: 'module' });
      ...
      let result = require('babel-core').transformFromAst(ast, precompiledSource, babelOptions);
      let compiledSource = result.code;
      // console.log(compiledSource);
      fs.ensureDirSync(path.dirname(path.join(this.outputPath, file)));
      fs.writeFileSync(path.join(this.outputPath, file), compiledSource);
    }
    ...
    for (let file of files) {
      ...
      let precompiledSource = fs.readFileSync(filePath, 'utf8');
      // babelOptions.filename = file;
      // let result = require('babel-core').transform(precompiledSource, babelOptions);
      // let compiledSource = result.code;
      let compiledSource = precompiledSource;
      fs.ensureDirSync(path.dirname(path.join(this.outputPath, file)));
      fs.writeFileSync(path.join(this.outputPath, file), compiledSource);
    }
  }
...
class CreateEntryPoint extends BroccoliPlugin {
  constructor(inputNode, options) {
    super(inputNode, {
      exclude: [],
    });
  ...
  build() {
    ...
    let code = files.reduce((code, file, i) => {
      let randomName = '';
      for (let j = 0; j <= i; j++) {
        randomName += 'a';
      }
      code += `export { default as ${randomName} } from './${file}';\n`;
      return code;
    }, '');

    fs.ensureDirSync(path.dirname(path.join(this.outputPath, outputFile)));
    fs.writeFileSync(path.join(this.outputPath, outputFile), code);
  }
}
...
class EmberApp {
  ...
  */
  ...
  _compileAddons(type) {
    let precompiledAddons = mergeTrees(this.addonTreesFor(type));

    let addonsToCompile = new Funnel(precompiledAddons, {
      exclude: 'ember-data',
    });

    let compiledAddons = preprocessJs(addonsToCompile, '/', '/', {
      registry: this.registry,
    });

    return mergeTrees([precompiledAddons, compiledAddons], {
      overwrite: true,
    });
  }
  ...
  */
  ...
  /**
    ...
    @return
  */
  _processedExternalTree(excludeAddons) {
  ...
  */
  appAndDependencies() {
    ...
    // let r = path.join(path.dirname(require.resolve('rollup-src')), '../..');

    // r = new Funnel(r, {
    //   exclude: ['node_modules'],
    // });

    // let bundle = new Rollup(r, {
    //   // nodeModulesPath: path.resolve(path.join(__dirname, '../../node_modules')),
    //   // nodeModulesPath: path.resolve(path.join(path.dirname(require.resolve('rollup-src')), '../../node_modules')),
    ...
    //   },
    // });
    // bundle = require('broccoli-stew').debug(bundle, { name: 'rollup' });

    let addonTree = new Funnel(mergeTrees(this.addonTreesFor('addon')), {
      destDir: 'addon-tree-output',
    });
    ...
    let entryPoint = new CreateEntryPoint([app], {
      outputFile: 'entryPoint.js',
    });

    let appAndAddons = mergeTrees([entryPoint, app, addonTree, config]);
    // let appAndAddonJs = new Funnel(appAndAddons, {
    //   include: ['**/*.js'],
    // });

    let compiledAppAndAddons = new Compile([appAndAddons], {
      appName: this.name,
      addons: this.project.addons,
      babelConfig: this.options,
      entryPoint: 'entryPoint.js',
    });
    // compiledAppAndAddons = mergeTrees([appAndAddons, compiledAppAndAddons], {
    //   overwrite: true,
    // });

    let preprocessedApp = new Funnel(compiledAppAndAddons, {
      include: [`${this.name}/**/*`],
    });
    ...
    // let entryPoint = concat(postprocessedApp, {
    //   inputFiles: [`**/*.js`],
    //   outputFile: 'entryPoint.js',
    // });

    // let addonTree = new Funnel(compiledAppAndAddons, {
    //   include: [`addon-tree-output/**/*`],
    // });

    // const Rollup = require('broccoli-rollup');

    // let asdf = mergeTrees([entryPoint, addonTree]);

    // asdf = require('broccoli-stew').debug(asdf, { name: 'before' });
    // compiledAppAndAddons = new Rollup(asdf, {
    //   rollup: {
    //     input: 'entryPoint.js',
    //     output: [
    //       { file: 'bundle.js', format: 'amd' },
    ...
    //   },
    // });
    // compiledAppAndAddons = require('broccoli-stew').debug(compiledAppAndAddons, { name: 'after' });

    compiledAppAndAddons = mergeTrees([compiledAppAndAddons, postprocessedApp], {
      overwrite: true,
    });
