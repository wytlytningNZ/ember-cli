...
class EmberApp {
  ...
  */
  ...
  compileAddonTrees(type, tree) {
    let trees = this.project.addons.reduce((sum, addon) => {
      if (addon.compile) {
        let val = addon.compile(type, new Funnel(tree, {
          srcDir: `addon-tree-output/${addon.moduleName()}`,
          // destDir: '/'
          allowEmpty: true,
        }));
        if (val) { sum.push(val); }
  ...
  */
  _processedExternalTree() {
    if (!this._cachedExternalTree) {
      ...
      // addons = require('broccoli-stew').debug(addons, { name: 'addons' });
  ...
  _compileExternalTree(tree) {
    return this.compileAddonTrees('addon', tree);
  }
  ...
  */
  appAndDependencies() {
    ...
    external = require('broccoli-stew').debug(external, { name: 'before' });
    external = this._compileExternalTree(external);
    external = require('broccoli-stew').debug(external, { name: 'after' });
