...
class Compile extends BroccoliPlugin {
  ...
  build() {
    ...
    let registry = this.options.registry;
    let everything = this.options.everything;
    ...
    amdFiles = amdFiles.map(file => file.replace(/^addon-tree-output\//, '').replace(/(\.js|\/)$/, '')).filter(Boolean);
    ...
    let external = [
      ...
      'rsvp',
      'require',
      path.join(appAndAddons, 'app-tree-output', appName, 'config/environment'), // './config/environment'
      `${appName}/config/environment`, // 'my-app/config/environment'
    ].concat(amdFiles);

    let include = this.options.include || [];

    include = include.map(i => path.join(appAndAddons, 'addon-tree-output', i));
    include.push(path.join(appAndAddons, 'app-tree-output', appName, '/**/*.js'));

    let destDir = this.outputPath;

    let nodeModulesSrc = path.join(everything.project.root, 'node_modules');
    let nodeModulesDest = path.join(destDir, 'node_modules');
    ...
    return rollup.rollup({
      ...
      plugins: [
        ...
        //   resolveId(importee) {
        //     // debugger;
        //     // return Promise.resolve(importee);
        //   },
        // },
        // {
        //   name: 'hbs',
        ...
        {
          moduleDest(file) {
            if (file.startsWith(nodeModulesSrc)) {
              let oldFile = file.substr(nodeModulesSrc.length + 1);
              return path.join(nodeModulesDest, oldFile);
            }
          },
        },
        {
          missingExport(module, name, otherModule) {
            ...
            console.log(`${module.id} cannot find '${name}' in ${otherModule.id}`);
      ...
      return bundle.write({
        ...
        excludedModules: ['\0rollup-plugin-multi-entry:entry-point'],
      });

      // bundle.bundle.orderedModules.forEach(module => {
      //   if (module.id.slice(1) === 'rollup-plugin-multi-entry:entry-point') {
      //     return;
      //   }
      //   let getPath = bundle.bundle.createGetPath({});
      //   let source = module.render(true, false, true, {
      //     preserveModules: true,
      ...
      //     let oldFile = module.id.substr(appAndAddons.length + 1);
      //     let newFile = path.join(this.outputPath, oldFile);
...
class EmberApp {
  ...
  */
  ...
  _addonTree() {
    if (!this._cachedAddonTree) {
      ...
      const X = require('./broccoli-module-normalizer');
      addonTrees = addonTrees.map(tree => new X(tree));
      ...
      // combinedAddonTree = require('broccoli-stew').debug(combinedAddonTree, { name: 'combinedAddonTree' });
  ...
  */
  appAndDependencies() {
    ...
    let external = this._processedExternalTree(true);
    ...
    //   registry: this.registry,
    // });
    // let preprocessedApp = new Funnel(app, {
    //   destDir: this.name,
    // });
    ...
    appAndAddons = require('broccoli-stew').debug(appAndAddons, { name: 'appAndAddons' });

    let strippedAppAndAddons;
    let treeShaking = this.options.treeShaking || {};
    if (treeShaking.enabled) {
      strippedAppAndAddons = new Compile([appAndAddons, amdModules], {
        appName: this.name,
        registry: this.registry,
      ...
      strippedAppAndAddons = appAndAddons;
    }

    strippedAppAndAddons = require('broccoli-stew').debug(strippedAppAndAddons, { name: 'strippedAppAndAddons' });

    let strippedApp = new Funnel(strippedAppAndAddons, {
      srcDir: 'app-tree-output',
      destDir: '/',
    });

    let strippedAddons = new Funnel(strippedAppAndAddons, {
      srcDir: 'addon-tree-output',
      destDir: '/',
    });

    let strippedNodeModules = new Funnel(strippedAppAndAddons, {
      srcDir: 'node_modules',
      destDir: '/',
    });

    let aliases = new NodeModulesMainAlias([strippedNodeModules], {
      projectRoot: this.project.root,
      annotation: 'NodeModulesMainAlias',
    });

    aliases = require('broccoli-stew').debug(aliases, { name: 'aliases' });

    aliases = new Funnel(aliases, {
      destDir: 'vendor',
    });

    let preprocessedApp = preprocessJs(strippedApp, '/', '/', {
      registry: this.registry,
    });

    let compiledAddons = preprocessJs(strippedAddons, '/', '/', {
      registry: this.registry,
    });

    let compiledNodeModules = preprocessJs(strippedNodeModules, '/', '/', {
      registry: this.registry,
    });

    compiledAddons = new Funnel(compiledAddons, {
      destDir: 'addon-tree-output',
    });

    compiledNodeModules = new Funnel(compiledNodeModules, {
    ...
    postprocessedApp = require('broccoli-stew').debug(postprocessedApp, { name: 'postprocessedApp' });

    let compiledAppAndAddons = mergeTrees([
      postprocessedApp,
      compiledAddons,
      compiledNodeModules,
      amdModules,
    ...
    compiledAppAndAddons = require('broccoli-stew').debug(compiledAppAndAddons, { name: 'compiledAppAndAddons' });

    let sourceTrees = [
      external,
      compiledAppAndAddons,
      config,
      emberCLITree,
      aliases,
    ];

    return require('broccoli-stew').debug(mergeTrees(sourceTrees, {
      overwrite: true,
