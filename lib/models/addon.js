...
let DEFAULT_COMPILE_METHODS = {
  app: 'compileApp',
  addon: 'compileAddon',
  'addon-styles': 'compileAddonStyles',
  'addon-templates': 'compileAddonTemplates',
  'addon-test-support': 'compileAddonTestSupport',
  public: 'compilePublic',
  styles: 'compileStyles',
  templates: 'compileTemplates',
  'test-support': 'compileTestSupport',
  vendor: 'compileVendor',
};
...
let addonProto = {
  ...
  */
  init(parent, project) {
    ...
    this.compileMethods = defaultsDeep({}, DEFAULT_COMPILE_METHODS);
  ...
  */
  ...
  compile(name, tree) {
    let compiledTree = this._compile(name, tree);
  ...
  */
  ...
  _compile(name, tree) {
    let compileMethod = this.compileMethods[name];

    if (this[compileMethod]) {
      tree = this[compileMethod](tree);
    }
  ...
  */
  ...
  treeForAddon(tree) {
    ...
    // tree = require('broccoli-stew').debug(tree, { name: 'before' });
    let addonTree = tree;
    // addonTree = require('broccoli-stew').debug(addonTree, { name: 'after' });

    let stylesTree = this._treeFor('addon-styles');

    tree = mergeTrees([addonTree, stylesTree], {
    ...
    return new Funnel(tree, {
      ...
      annotation: 'Funnel: moduleName',
    });
  },

  compileAddon(tree) {
    return this._compileAddon(tree);
  },
  ...
  */
  processedAddonJsFiles(addonTree) {
    ...
    preprocessedAddonJS = require('broccoli-stew').debug(preprocessedAddonJS, { name: `${this.name}-preprocessedAddonJS` });
    let processedAddonJS = this.preprocessJs(preprocessedAddonJS, '/', this.name, {
      annotation: `processedAddonJsFiles(${this.name})`,
      registry: this.registry,
    });
    processedAddonJS = require('broccoli-stew').debug(processedAddonJS, { name: `${this.name}-processedAddonJS` });
