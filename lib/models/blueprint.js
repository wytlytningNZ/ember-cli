...
function getPackageName(packageObject) {
  return packageObject.name;
}

function getPackageNameAndVersion(delimiter, packageObject) {
  var packageNameAndVersion = getPackageName(packageObject);

  if (packageObject.target) {
    packageNameAndVersion += delimiter + packageObject.target;
  }

  return packageNameAndVersion;
}

function createPackage(packageName, target) {
...
function mapPackages(packages, delimiter) {
  return packages.map(getPackageNameAndVersion.bind(null, delimiter));
}
...
Blueprint.prototype.addPackageToProject = function(packageName, target) {
  var packageObject = createPackage(packageName, target);
...
Blueprint.prototype.addPackagesToProject = function(packages) {
  ...
  this._writeStatusToUI(chalk.green, install, packages.map(getPackageName).join(', '));
...
function getTaskOptions(options) {
  var taskOptions = {};
...
Blueprint.prototype.addAddonToProject = function(options) {
  var taskOptions = getTaskOptions(options);
...
function getInsertBehavior(options) {
  if (options.before) { return 'before'; }
  if (options.after)  { return 'after'; }
  return 'end';
}

function shouldInsert(alreadyPresent, force) {
  return force || !alreadyPresent;
}

function getOriginalContents(fullPath) {
  if (!fs.existsSync(fullPath)) { return ''; }
  return fs.readFileSync(fullPath, { encoding: 'utf8' });
}

function getContentsToWrite(originalContents, contentsToInsert, providedOptions) {
  ...
  var insertBehavior  = getInsertBehavior(options);

  if (shouldInsert(alreadyPresent, options.force)) {
...
Blueprint.prototype.insertIntoFile = function(pathRelativeToProjectRoot, contentsToInsert, providedOptions) {
  ...
  var originalContents = getOriginalContents(fullPath);
  var contentsToWrite  = getContentsToWrite(originalContents, contentsToInsert, providedOptions);
...
Blueprint.prototype.printDetailedHelp = function() {
  if (this.path) {
    var filePath = path.join(this.path, './HELP.md');
    if (fs.existsSync(filePath)) {
      return new MarkdownColor().renderFile(filePath, {indent:'        '});
    }
  }
  return '';
};
...
Blueprint.lookup = function(name, options) {
  ...
  for (var i = 0, lookupPath; lookupPath = lookupPaths[i]; i++) {
...
function getSource(packagePath, lookupPath) {
  if (fs.existsSync(packagePath)) {
    return require(packagePath).name;
  }
  return path.basename(path.join(lookupPath, '..'));
}

function mapBlueprints(blueprints, seen) {
  return blueprints.map(function(blueprintPath) {
...
Blueprint.list = function(options) {
  ...
  return lookupPaths.map(function(lookupPath) {
    ...
    return {
      source: getSource(packagePath, lookupPath),
      blueprints: mapBlueprints(blueprints, seen)
    };
  });
};
